...
//
//public class MFDR_BAK_with_DFTForMFDR extends DimensionalityReduction {
//	private static Log logger = LogFactory.getLog(MFDR_BAK_with_DFTForMFDR.class);
...
//	 * @param weight_freq
//	 */
//	public MFDR_BAK_with_DFTForMFDR(int NoC_t, int NoC_s) {
...
//	}
//
//	public double[] computeIndexingQuery(TimeSeries query, LinkedList<MFDRWaveData> mfdrlist, Distance distance, int NoC){
//		return null;
////		PLA pla_q = new PLA(0);
...
////		// Extract all possible trends
////		for(int i = 0 ; i <= NoC ; i++){
////			pla_q.setNoC(i);
////			q_trend.add(pla_q.getDR(query));
////			TimeSeries q_trendfull = pla_q.getFullResolutionDR(q_trend.get(i),query);
////			q_residualfull.add(DataListOperator.getInstance().linkedtListSubtraction(query, q_trendfull));
////		}
////		for(int i = NoC; i >=0 ; i--){
////			DFT dft_q = new DFT
////		}
...
//		TimeSeries trendfull = this.pla.getFullResolutionDR(trends,ts);
...
//		d.getMeta().setResidualFull(residualfull);
//		return d;
//	}
//
//	/**
...
//				size);
//		double noiseenergy = mfdrdata1.noiseEnergyDensity()
//				+ mfdrdata2.noiseEnergyDensity();
...
//		double dist_trend = pla.getDistance(lcmtrends.get(0), lcmtrends.get(1),
//				size, distance);
